...
#if defined(NDEBUG)
# error "Imperialcredit cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x33aa19ff861103009dc2c261f8403530009b50cb188f60d73a9f1ae2bd15f637");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Imperialcredit: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Imperialcredit Signed Message:\n";
...
{
    // Imperialcredit: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 250000); // Imperialcredit: 

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 3.5 * 24 * 60 * 60; // Imperialcredit: 3.5 days
static const int64 nTargetSpacing = 1 * 60; // Imperialcredit: 1 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Imperialcredit: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Imperialcredit: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0xec253c496660e8e774be297b10658604a59c9bb5d9259e6345eae9f3ee80afa8");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("042588f060e103928b454b6c6c32947edc83095085661213e4426770505356820f37d980fb3b8caf25916f05ead35c6136c8c347c9f36a8c84dd5640257e2ac383") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1521284368;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2087497499;

        if (fTestNet)
        {
            block.nTime    = 1521284294;
            block.nNonce   = 387707718;
        }
...
if (false && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0x75c1efde43129d3da6ccf31caf21372cdb30fbdcb616288139f3053e69d04945"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfc, 0xc4, 0xb1, 0xdd }; // Imperialcredit: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("ImperialcreditMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("ImperialcreditMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static ImperialcreditMiner(CWallet *pwallet)
{
    printf("ImperialcreditMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("imperialcredit-miner");
    ...
    try { loop {
        ...
        printf("Running ImperialcreditMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("ImperialcreditMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&ImperialcreditMiner, pwallet));
}
